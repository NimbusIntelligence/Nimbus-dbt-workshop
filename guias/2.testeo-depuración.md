...
3. Crea un archivo sources.yml en la carpeta library_loans (/models/library_loans/sources.yml) y copia/pega el siguiente SQL dentro de él.

    <details>
    <summary>Pista</summary>
    recuerda que el archivo debe seguir este formato:

    ```yml
    version: 2

    sources:
      - name: Nombre de tu source (puede ser cualquiera pero preferiblemente algo descriptivo)
        database: Nombre de tu base de datos(exactamente igual como aparece en snowflake)
        schema: Nombre de tu schema (exactamente igual como aparece en snowflake)
        tables:
          - name: nombre de la tabla(exactamente igual como aparece en snowflake)
          - name: ...
          - name: ...
    ```
    </details>
...
4. Actualiza la sección de models del archivo dbt_project.yml (líneas 38-45) para que todo lo que esté en library_loans se materialice por defecto como tablas.
    ...
    recuerda que el archivo en la seccion de models debe seguir este formato:
...
5. Actualiza customers_with_late_fees.sql para que utilice las fuentes definidas (sources) en lugar de los valores codificados directamente.
    ...
    <details>
    <summary>Pista</summary>
    Usa el jinja adecuado para insertar tus fuentes en el SQL, puedes usar el comando __sources para obtener la estrcutura adecuada
    </details>
...
- test de unique y not null para:
  ...
  - loans.loan_id
    ...
    recuerda que el archivo debe llevar el siguiente formato:

    ```yml
      version: 2

    sources:
      - name: Nombre de tu sources (puede ser cualquiera pero preferiblemente algo descriptivo)
        database: Nombre de tu base de datos(exactamente igual como aparece en snowflake)
        schema: Nombre de tu schema (exactamente igual como aparece en snowflake)
        tables:
            - name: nombre de la tabla(exactamente igual como aparece en snowflake)
              columns:
                - name: nombre de la columna(exactamente igual como aparece en snowflake)
                  data_tests:
                    - test que deseas aplicar
                    - ...
            - name: ...
    ```
    </details>
...
11. Crea un archivo schema.yml de library_loans, elimina las pruebas de members (líneas 26-33) del sources.yml y aplícalas al nuevo stg_members dentro de un bloque de models en el archivo schema.yml.
...
15. En el archivo sources.yml de library_loans, elimina las pruebas de book_id (líneas 9-13 y 15-19) y aplica una sola prueba de book_id no nula y única al nuevo modelo stg_books en la sección de modelos en el archivo schema.yml.

    <details>
    <summary>Respuesta sources.yml</summary>
    ...
    <details>
    <summary>Respuesta schema.yml</summary>
...
17. En sources.yml agrega pruebas de relación a nuestra fuente de préstamos (loans):
...
32. Crea un archivo en la raiz del proyecto llamado packages.yml.
